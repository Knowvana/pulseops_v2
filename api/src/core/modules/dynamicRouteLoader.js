...
export async function loadModuleRoutes(app, moduleId) {
  ...
  try {
    ...
    logger.info(`[DynamicRouteLoader] Module '${moduleId}' API routes loaded at ${mountPath}`);
    return { success: true, message: `Module '${moduleId}' API routes loaded at ${mountPath}.` };
  } catch (err) {
